...
const add_=()=>
{
	...
	if(argss[3])
	{
		...
		if(fs.existsSync("task.txt"))
			...
			fs.writeFileSync("task.txt",read,(err)=>{if(err) throw(err);});
			}
		}
		else
		    {
...
const report_=()=>
{
	...
	read_done=read_done.filter(function(e){return e;});
		...
		read_done[i].split(i);
		let a=read_done[i].indexOf(' ');
	console.log((i+1) + "." + " "+ read_done[i].substring(a+1,read_done[i].length));
		   }
...
const done_=()=>
{
	if(argss[3])
	{
	   ...
	   else{console.log("Error: no incomplete item with index \#"+argss[3]+" exists.");}
